...
class ProjectController extends Controller
{
    ...
    function investorSenddocmailtomanager(Request $request)
    {
        ...
        Mail::send('emails.drawingsubmission', $emaildata, function($message)use($emaildata,$doc_header,$cc) {
            $message->to($emaildata['tenant_email'])
                    ->cc($cc)
                    ->subject($emaildata["unit_name"]."-".$emaildata["property_name"]."-".$emaildata["investor_brand"]."-".$doc_header."- Drawings Submission");
            });
        if(Mail::failures())
        {
            return response()->json(['response'=>"Document notify Mail Not Sent"], 410);
        ...
        // {
            ...
            $emaildata['docs']=$documents;
             Mail::send('emails.drawingsubmission', $emaildata, function($message)use($emaildata,$tenant_details) {
                $message->to($tenant_details[0]['tenant_email'])
    ...
    function testing(Request $request)
    {
        ...
        $data = Project::where('project_id',38)->get();
   //  $data = Projectdocs::where('project_id',38)->get();

      //  $data =  ProjectdocsApproval::where('project_id',36)->get();
        echo "<pre>";
        print_r($data);
        die;
       // return $data;
