...
class LicenseRemainder extends Command
{
     ...
     * @var string
     */
    protected $signature = 'license:remainder';
    ...
    {
        $date_limit = Carbon::today()->addDays(45);
        ...
        ->leftjoin('users as e','a.member_id','=','e.mem_id')
        ...
        ->leftjoin('users as f','c.member_id','=','f.mem_id')
        ...
        ->whereDate('tbl_project_investor_planned_dates.fitout_completion', '=', $date_limit)
        ->select('tbl_projects.project_id','tbl_projects.project_name','a.member_designation as rdd_manager_designation','a.email as rdd_manager_email','b.member_designation as investor_designation','b.email as investor_email','c.member_designation as contractor_designation','tbl_units_master.unit_name','tbl_properties_master.property_name','e.mem_name','e.mem_last_name','d.tenant_name as investor_first_name','d.tenant_last_name as investor_last_name','tbl_project_investor_planned_dates.fitout_completion','c.email as rdd_admin_email')
        ...
        for($i=0;$i<count($projectDetails);$i++)
         {
                ...
                $to = array();
                $cc = array();
                array_push($to,$projectDetails[$i]['investor_email']);
                array_push($cc,$projectDetails[$i]['rdd_admin_email'],$emaildata['rdd_manager_email']);
                Mail::send('reminders.licenseremainder', $emaildata, function($message)use($emaildata,$to,$cc) {
                    $message->to($to)
                            ->cc($cc)
                            ->subject($emaildata['unit_name']."-".$emaildata['investor_brand']."-".$emaildata['property_name']."-Required License Remainder");

                          //  ->subject($emaildata['unit_name']."-".$emaildata['property_name']."-".$emaildata['project_name']."-Required License_Remainder");
                    }); 
         }
    }
}
